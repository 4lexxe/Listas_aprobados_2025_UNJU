...
function updateResultsTable(results) {
    ...
    tableBody.innerHTML = '';
    modalTableBody.innerHTML = ''; existan antes de manipularlos
    if (tableBody) tableBody.innerHTML = '';
    if (results.length === 0) {bleBody.innerHTML = '';
        ...
        // Mostrar mensaje de "No aprobado o no registrado" en la tabla
        const noResultRow = `ableContainer.style.display = 'block';
            <tr>
                <td colspan="5" class="text-center">
                    <span class="badge bg-danger">No aprobado o no registrado</span>
                </td>olspan="5" class="text-center">
            </tr>   <span class="badge bg-danger">No aprobado o no registrado</span>
        `;      </td>
            </tr>
        tableBody.innerHTML = noResultRow;
        modalTableBody.innerHTML = noResultRow;
        if (tableBody) tableBody.innerHTML = noResultRow;
        // Mostrar el botón de ver modal solo si hay resultadosRow;
        const viewModalBtn = document.getElementById('view-results-modal-btn');
        // Mostrar el botón de ver modal solo si hay resultados
        const viewModalBtn = document.getElementById('view-results-modal-btn');
        if (viewModalBtn) {
            viewModalBtn.style.display = 'none';
        }
    ...
    // Crear las filas de la tabla para ambas tablas (normal y modal)
    const rows = [];
    ...
    // Actualizar ambas tablas si existen
    if (tableBody) tableBody.innerHTML = rows.join('');
    if (modalTableBody) modalTableBody.innerHTML = rows.join('');
    
    if (tableContainer) tableContainer.style.display = 'block';
    
    // Mostrar el botón de ver modal si hay resultados
    const viewModalBtn = document.getElementById('view-results-modal-btn');
    if (viewModalBtn) {
        viewModalBtn.style.display = 'inline-block';
    }
}
...
});jecutará cuando todo el DOM esté cargado

// Event listener para el botón de ver resultados en modaltById('view-results-modal-btn');
document.addEventListener('DOMContentLoaded', () => {
    const viewModalBtn = document.getElementById('view-results-modal-btn');EventListener('click', () => {
    if (viewModalBtn) {         try {
        viewModalBtn.addEventListener('click', () => {                const resultsModal = document.getElementById('resultsModal');
            try {sultsModal && window.bootstrap && bootstrap.Modal) {
                // Usar Bootstrap para mostrar el modalew bootstrap.Modal(resultsModal);
                const resultsModal = new bootstrap.Modal(document.getElementById('resultsModal'));           modal.show();
                resultsModal.show();
            } catch (error) {Bootstrap no disponible');
...
// Contador de visitas        console.warn('Botón de ver modal no encontrado en el DOM');
...
}    }        document.getElementById('visits').innerText = 'Error';        console.error('Error al actualizar el contador de visitas:', error);    } catch (error) {        document.getElementById('visits').innerText = data.value.toLocaleString();        const data = await response.json();        const response = await fetch('https://api.countapi.xyz/hit/listas2025-alexxe/visits');    try {async function updateVisitCount() {    }
});
